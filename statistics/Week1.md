...
통계학의 필요성
통계학이라는 기본기는 데이터 과학의 기반이 된다. <br> 데이터를 수집/가공하고, EDA를 수행하고, 가설을 설정하고 검정하고 결과를 해석하고 적용하는 모든 과정이 우리가 "관심있어하는" 문제에 대한 해답으로 가는 가장 빠른 길이다. 이는 전반적으로 통계학 지식이 없이는 불가능한 것들이다.
